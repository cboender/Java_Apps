...
import static sample.gears.GLFWUtil.*;
...
public class Gears extends AbstractGears {
	...
	private void init() {
		...
		/*
		 * // This code did the equivalent of
		 * glfwWindowHint(GLFW_COCOA_RETINA_FRAMEBUFFER, GLFW_FALSE) before GLFW 3.3 if
		 * ( Platform.get() == Platform.MACOSX ) { long cocoaWindow =
		 * glfwGetCocoaWindow(window);
		 * 
		 * long objc_msgSend =
		 * ObjCRuntime.getLibrary().getFunctionAddress("objc_msgSend"); long contentView
		 * = invokePPP(cocoaWindow, sel_getUid("contentView"), objc_msgSend);
		 * 
		 * invokePPV(contentView, sel_getUid("setWantsBestResolutionOpenGLSurface:"),
		 * false, objc_msgSend);
		 * 
		 * boolean bool = invokePPZ(contentView,
		 * sel_getUid("wantsBestResolutionOpenGLSurface"), objc_msgSend);
