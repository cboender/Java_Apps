 ...
 *
 */
public class Scene5_Transform extends Scene {
	...
	protected void init() {
		...
		texture.initialize();
		texture2 = new Texture("PNG", "smile.jpg", GL13.GL_TEXTURE1);
		texture2.initialize();

		// Shaders
		shaderProgram = new ShaderProgram(new TransformVertexShader(), new TransformFragmentShader());
	...
	protected void loop() {
		...
		Matrix4 translate = Matrix4.createTranslate((float) Math.sin(GLFW.glfwGetTime()), -.5f, 0f);
		Matrix4 rotate = Matrix4.createRotation(-GLFW.glfwGetTime(), 0, 0, 1);
		...
		shaderProgram.setUniformMatrix("transform", Matrix4.merge(scale, translate, rotate).getMatrix());
		GL11.glDrawElements(GL11.GL_TRIANGLES, 6, GL11.GL_UNSIGNED_INT, 0);

		translate.setValue(3, 0, Math.sin(GLFW.glfwGetTime()));
		translate.setValue(3, 1, .5f);
		scale.setValue(0, 0, 1f);
		shaderProgram.setUniformMatrix("transform", Matrix4.merge(scale, translate, rotate).getMatrix());
		GL11.glDrawElements(GL11.GL_TRIANGLES, 6, GL11.GL_UNSIGNED_INT, 0);
		GL30.glBindVertexArray(0);
	}
